...
1) [Download](https://github.com/WebDevStudios/wd_s/archive/master.zip) and extract the zip into your `wp-content/themes` directory and rename `wdzemoga-master` to fit your needs.

2) Find & Replace

You'll need to change all instances of the names: `_s`.

* Search for: `'zemoga'` and replace with: `'project-name'` (inside single quotations) to capture the text domain
* Search for: `zemoga_` and replace with: `project-name_` to capture all the function names
* Search for: `Text Domain: zemoga` and replace with: `Text Domain: project-name` in style.css
* Search for (and include the leading space): <code>&nbsp;_s</code> and replace with: <code>&nbsp;Project Name</code> (with a space before it) to capture DocBlocks
* Search for: `zemoga-` and replace with: `project-name-` to capture prefixed handles
* Search for `zemoga.pot` and replace with: `project-name.pot` to capture translation files
* Search for `zemoga.dev` and replace with: `project-name.dev` to match your local development URL
...
```
![Install and Gulp](https://dl.dropboxusercontent.com/s/cj1p6xjz51cpckq/wdzemoga-install.gif?dl=0)
