...
if ( ! function_exists( 'zemoga_posted_on' ) ) :
	/**
	 * Prints HTML with meta information for the current post-date/time and author.
	 */
	function zemoga_posted_on() {
		...
		$posted_on = sprintf(
			...
			esc_html_x( 'Posted on %s', 'post date', 'zemoga' ),
			'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
		);

		$byline = sprintf(
			/* translators: the post author */
			esc_html_x( 'by %s', 'post author', 'zemoga' ),
...
if ( ! function_exists( 'zemoga_entry_footer' ) ) :
	/**
	 * Prints HTML with meta information for the categories, tags and comments.
	 */
	function zemoga_entry_footer() {
		// Hide category and tag text for pages.
		if ( 'post' === get_post_type() ) {
			/* translators: used between list items, there is a space after the comma */
			$categories_list = get_the_category_list( esc_html__( ', ', 'zemoga' ) );
			if ( $categories_list && zemoga_categorized_blog() ) {
				/* translators: the post category */
				printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'zemoga' ) . '</span>', $categories_list ); // WPCS: XSS OK.
			}

			/* translators: used between list items, there is a space after the comma */
			$tags_list = get_the_tag_list( '', esc_html__( ', ', 'zemoga' ) );
			if ( $tags_list ) {
				/* translators: the post tags */
				printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'zemoga' ) . '</span>', $tags_list ); // WPCS: XSS OK.
			}
		}

		if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
			echo '<span class="comments-link">';
			comments_popup_link( esc_html__( 'Leave a comment', 'zemoga' ), esc_html__( '1 Comment', 'zemoga' ), esc_html__( '% Comments', 'zemoga' ) );
			echo '</span>';
		}
 ...
 */
function zemoga_display_svg( $args = array() ) {
	echo zemoga_get_svg( $args ); // WPCS XSS Ok.
}
 ...
 * @return string
 */
function zemoga_get_svg( $args = array() ) {

	// Make sure $args are an array.
	if ( empty( $args ) ) {
		return esc_html__( 'Please define default parameters in the form of an array.', 'zemoga' );
	}

	// Define an icon.
	if ( false === array_key_exists( 'icon', $args ) ) {
		return esc_html__( 'Please define an SVG icon filename.', 'zemoga' );
	}
	...
	<svg
	<?php
		echo zemoga_get_the_content( $height ); // WPCS XSS OK.
		echo zemoga_get_the_content( $width ); // WPCS XSS OK.
		echo zemoga_get_the_content( $fill ); // WPCS XSS OK.
	?>
		class="icon icon-<?php echo esc_attr( $args['icon'] ); ?>"
	<?php
		echo zemoga_get_the_content( $aria_hidden ); // WPCS XSS OK.
		echo zemoga_get_the_content( $aria_labelledby ); // WPCS XSS OK.
	?>
		role="img">
 ...
 * @return string
 */
function zemoga_get_the_title( $args = array() ) {
 ...
 * @return string
 */
function zemoga_get_the_excerpt( $args = array() ) {
 ...
 * @return string
 */
function zemoga_display_post_image( $size = 'thumbnail' ) {
	...
	$attached_image_url = zemoga_get_attached_image_url( $size );
 ...
 * @return string
 */
function zemoga_get_post_image_url( $size = 'thumbnail' ) {
	...
	return zemoga_get_attached_image_url( $size );
}
 ...
 * @return string
 */
function zemoga_get_attached_image_url( $size = 'thumbnail' ) {
 ...
 * @return bool
 */
function zemoga_display_copyright_text() {

	// Grab our customizer settings.
	$copyright_text = get_theme_mod( 'zemoga_copyright_text' );
	...
	echo zemoga_get_the_content( do_shortcode( $copyright_text ) ); // phpcs: xss ok.
}
 ...
 * @return string
 */
function zemoga_get_twitter_share_url() {
 ...
 * @return string
 */
function zemoga_get_facebook_share_url() {
 ...
 * @return string
 */
function zemoga_get_linkedin_share_url() {
	return add_query_arg(
		array(
 ...
 */
function zemoga_display_social_network_links() {
	...
	<ul class="social-icons">
		...
		foreach ( $social_networks as $network ) :
			...
			$network_url = get_theme_mod( 'zemoga_' . $network . '_link' );
			...
			?>
				<li class="social-icon <?php echo esc_attr( $network ); ?>">
					<a href="<?php echo esc_url( $network_url ); ?>">
						...
						<?php
							echo /* translators: the social network name */ sprintf( esc_html( 'Link to %s', 'zemoga' ), ucwords( esc_html( $network ) ) ); // WPCS: XSS OK.
						?>
						</span>
					</a>
				</li><!-- .social-icon -->
 ...
 */
function zemoga_display_card( $args = array() ) {
	...
	<div class="<?php echo esc_attr( $args['class'] ); ?> card">
		...
		<?php if ( $args['url'] ) : ?>
			<a class="button button-card" href="<?php echo esc_url( $args['url'] ); ?>"><?php esc_html_e( 'Read More', 'zemoga' ); ?></a>
		<?php endif; ?>
 ...
 * @return string
 */
function zemoga_display_header_button() {

	// Get our button setting.
	$button_setting = get_theme_mod( 'zemoga_header_button' );
	...
	$button_url  = get_theme_mod( 'zemoga_header_button_url' );
	$button_text = get_theme_mod( 'zemoga_header_button_text' );
	?>
	<div class="site-header-action">
		...
		?>
			<a href="<?php echo esc_url( $button_url ); ?>" class="button button-link"><?php echo esc_html( $button_text ?: __( 'More Information', 'zemoga' ) ); ?></a>
		<?php else : ?>
			<button type="button" class="cta-button" aria-expanded="false" aria-label="<?php esc_html_e( 'Search', 'zemoga' ); ?>">
				<?php esc_html_e( 'Search', 'zemoga' ); ?>
			</button>
 ...
 * @author Corey Collins
 */
function zemoga_display_numeric_pagination( $args = array() ) {
	...
	<nav class="pagination-container container" aria-label="<?php esc_html_e( 'numeric pagination', 'zemoga' ); ?>">
 ...
 * @author Corey Collins
 */
function zemoga_display_mobile_menu() {
	...
	<nav class="off-canvas-container" aria-label="<?php esc_html_e( 'Mobile Menu', 'zemoga' ); ?>" aria-hidden="true" tabindex="-1">
		<button type="button" class="off-canvas-close" aria-label="<?php esc_html_e( 'Close Menu', 'zemoga' ); ?>">
