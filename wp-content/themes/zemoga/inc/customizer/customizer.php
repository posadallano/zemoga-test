 ...
 */
function zemoga_remove_default_customizer_sections( $wp_customize ) {
...
add_action( 'customize_register', 'zemoga_remove_default_customizer_sections', 15 );

/**
 * Include other customizer files.
 */
function zemoga_include_custom_controls() {
...
add_action( 'customize_register', 'zemoga_include_custom_controls', -999 );

/**
 * Enqueue customizer related scripts.
 */
function zemoga_customize_scripts() {
	wp_enqueue_script( 'zemoga-customize-livepreview', get_template_directory_uri() . '/inc/customizer/assets/scripts/livepreview.js', array( 'jquery', 'customize-preview' ), '1.0.0', true );
}
add_action( 'customize_preview_init', 'zemoga_customize_scripts' );
 ...
 */
function zemoga_selective_refresh_support( $wp_customize ) {

	// The <div> classname to append edit icon too.
	$settings = array(
		'blogname'          => '.site-title a',
		'blogdescription'   => '.site-description',
		'zemoga_copyright_text' => '.site-info',
	);
...
add_action( 'customize_register', 'zemoga_selective_refresh_support' );
 ...
 */
function zemoga_live_preview_support( $wp_customize ) {
...
add_action( 'customize_register', 'zemoga_live_preview_support', 999 );
