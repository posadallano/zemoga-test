 ...
 * @return array
 */
function zemoga_body_classes( $classes ) {
...
add_filter( 'body_class', 'zemoga_body_classes' );

/**
 * Flush out the transients used in zemoga_categorized_blog.
 *
 * @return string
 */
function zemoga_category_transient_flusher() {
	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
		return false;
	}
	// Like, beat it. Dig?
	delete_transient( 'zemoga_categories' );
}
add_action( 'delete_category', 'zemoga_category_transient_flusher' );
add_action( 'save_post', 'zemoga_category_transient_flusher' );
 ...
 * @return string
 */
function zemoga_content_more_link() {
	return ' <a class="more-link" href="' . get_permalink() . '">' . esc_html__( 'Read More', 'zemoga' ) . '...</a>';
}
add_filter( 'the_content_more_link', 'zemoga_content_more_link' );
 ...
 * @return string
 */
function zemoga_excerpt_more( $more ) {
	return sprintf( ' <a class="more-link" href="%1$s">%2$s</a>', get_permalink( get_the_ID() ), esc_html__( 'Read more...', 'zemoga' ) );
}
add_filter( 'excerpt_more', 'zemoga_excerpt_more' );
 ...
 * @return array
 */
function zemoga_custom_mime_types( $mimes ) {
	$mimes['svg']  = 'image/svg+xml';
	$mimes['svgz'] = 'image/svg+xml';
	return $mimes;
}
add_filter( 'upload_mimes', 'zemoga_custom_mime_types' );
...
add_shortcode( 'svg', 'zemoga_display_svg' );
 ...
 * @return string
 */
function zemoga_display_customizer_header_scripts() {

	// Check for header scripts.
	$scripts = get_theme_mod( 'zemoga_header_scripts' );
	...
	echo zemoga_get_the_content( $scripts ); // WPCS XSS OK.
}
add_action( 'wp_head', 'zemoga_display_customizer_header_scripts', 999 );
 ...
 * @return string
 */
function zemoga_display_customizer_footer_scripts() {

	// Check for footer scripts.
	$scripts = get_theme_mod( 'zemoga_footer_scripts' );
...
add_action( 'wp_footer', 'zemoga_display_customizer_footer_scripts', 999 );
...
function zemoga_add_og_tags() {
	...
	$default_base_description = ( get_bloginfo( 'description' ) ) ? get_bloginfo( 'description' ) : esc_html__( 'Visit our website to learn more.', 'zemoga' );
	...
	if ( is_category() || is_tag() || is_tax() ) {
		...
		$specify        = ( is_category() ) ? esc_html__( 'categorized in', 'zemoga' ) : esc_html__( 'tagged with', 'zemoga' );
		$queried_object = get_queried_object();
		$card_url       = get_term_link( $queried_object );
		$card_type      = 'website';

		// Translators: get the term name.
		$card_long_description = $card_description = sprintf( esc_html__( 'Posts %1$s %2$s.', 'zemoga' ), $specify, $term_name );
	}
	...
	if ( is_search() ) {
		...
		$card_long_description = $card_description = sprintf( esc_html__( 'Search results for %s.', 'zemoga' ), $search_term );
	}
...
add_action( 'wp_head', 'zemoga_add_og_tags' );
 ...
 * @author Corey Collins
 */
function zemoga_remove_archive_title_prefix( $title ) {
