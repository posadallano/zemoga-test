 ...
 */
function zemoga_display_block_options( $args = array() ) {
	...
	if ( $args['background_type'] ) {
		...
		if ( 'video' === $args['background_type'] ) {
			...
			$background_alt = $background_title ? sprintf( esc_attr( 'Video Background of %s', 'zemoga' ), esc_attr( $background_options['background_video_title'] ) ) : __( 'Video Background', 'zemoga' );
			...
			?>
				...
				<button class="video-toggle"><span class="screen-reader-text"><?php esc_html_e( 'Toggle video playback', 'zemoga' ); ?></span></button>
			<?php
 ...
 */
function zemoga_get_animation_class() {
 ...
 * @return bool
 */
function zemoga_has_block_expired( $args = array() ) {
 ...
 */
function zemoga_acf_flexible_content_layout_title( $title, $field, $layout, $i ) {
	...
	if ( ! 'none' === $background ) {
		...
		$type = zemoga_return_flexible_content_layout_value( $background_type );
		...
		if ( 'video' === $background_type ) {
			$title .= '<div style="font-size: 30px; height: 26px; width: 30px;" class="dashicons dashicons-format-video acf-flexible-title-image"><span class="screen-reader-text">' . esc_html__( 'Video', 'zemoga' ) . '</span></div>';
		}
	}
	...
	if ( zemoga_has_block_expired(
		...
		$expired .= '<span style="color: red;">&nbsp;(' . esc_html__( 'expired', 'zemoga' ) . ')</span>';
	}
...
add_filter( 'acf/fields/flexible_content/layout_title/name=content_blocks', 'zemoga_acf_flexible_content_layout_title', 10, 4 );
 ...
 */
function zemoga_return_flexible_content_layout_value( $type ) {
...
if ( function_exists( 'zemoga_acf_flexible_content_layout_title' ) ) {

	/**
	 * Set Admin Styles for Flexible Content Layout Image/Title in zemoga_acf_flexible_content_layout_title().
	 */
	function zemoga_flexible_content_layout_title_acf_admin_head() {
	?>
	<style type="text/css">
	...
	add_action( 'acf/input/admin_head', 'zemoga_flexible_content_layout_title_acf_admin_head' );
}

/**
 * Load colors dynamically into select field from zemoga_get_theme_colors()
 *
 ...
 */
function zemoga_acf_load_color_picker_field_choices( $field ) {

	// Reset choices.
	$field['choices'] = array();

	// Grab our colors array.
	$colors = zemoga_get_theme_colors();
...
add_filter( 'acf/load_field/name=color_picker', 'zemoga_acf_load_color_picker_field_choices' );
 ...
 */
function zemoga_get_theme_colors() {
	return array(
		esc_html__( 'Alto', 'zemoga' )           => '#ddd',
		esc_html__( 'Black', 'zemoga' )          => '#000',
		esc_html__( 'Blue', 'zemoga' )           => '#21759b',
		esc_html__( 'Cod Gray', 'zemoga' )       => '#111',
		esc_html__( 'Dove Gray', 'zemoga' )      => '#666',
		esc_html__( 'Gallery', 'zemoga' )        => '#eee',
		esc_html__( 'Gray', 'zemoga' )           => '#808080',
		esc_html__( 'Gray Alt', 'zemoga' )       => '#929292',
		esc_html__( 'Light Yellow', 'zemoga' )   => '#fff9c0',
		esc_html__( 'Mineshaft', 'zemoga' )      => '#333',
		esc_html__( 'Silver', 'zemoga' )         => '#ccc',
		esc_html__( 'Silver Chalice', 'zemoga' ) => '#aaa',
		esc_html__( 'White', 'zemoga' )          => '#fff',
		esc_html__( 'Whitesmoke', 'zemoga' )     => '#f1f1f1',
	);
}
 ...
 * @return void
 */
function zemoga_display_hero_heading( $title ) {
