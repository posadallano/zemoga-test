 ...
 */
function zemoga_categorized_blog() {

	$category_count = get_transient( 'zemoga_categories' );
	...
	if ( false === $category_count ) {
		...
		set_transient( 'zemoga_categories', $category_count );
	}
 ...
 */
function zemoga_get_attachment_id_from_url( $attachment_url = '' ) {
 ...
 * @return string
 */
function zemoga_get_placeholder_unsplash( $args = array() ) {
	...
	if ( ! is_numeric( $args['width'] ) ) {
		throw new Exception( esc_html__( 'Width must be an integer', 'zemoga' ) );
	}

	if ( ! is_numeric( $args['height'] ) ) {
		throw new Exception( esc_html__( 'Height must be an integer', 'zemoga' ) );
	}
	...
	$alt = apply_filters( 'zemoga_placeholder_image_alt', esc_html__( 'WebDevStudios Placeholder Image', 'zemoga' ) );
 ...
 * @return string
 */
function zemoga_copyright_year( $atts ) {
...
add_shortcode( 'zemoga_copyright_year', 'zemoga_copyright_year', 15 );
...
if ( defined( 'WPSEO_VERSION' ) ) {
	 ...
	 * @since  NEXT
	 */
	function zemoga_move_yoast_to_bottom() {
		return 'low';
	}
	add_filter( 'wpseo_metabox_prio', 'zemoga_move_yoast_to_bottom' );
}
 ...
 * @author jomurgel
 */
function zemoga_get_the_content( $content ) {
...
add_filter( 'the_content', 'zemoga_get_the_content', 20 );
 ...
 */
function zemoga_get_theme_colors_gutenberg() {
